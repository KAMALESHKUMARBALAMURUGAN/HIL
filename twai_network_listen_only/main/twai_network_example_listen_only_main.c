...
printf("ingi---------------->%d \t,%d\t,%d\t,%d\t,%d\t",ingi,brake,modeL,modeR,reve);
...
rpm = 3350;
  // Extract the first two digits
    ...
    // Convert rpm1 and rpm2 to hexadecimal strings and store them in variables
    sprintf(rpm1_hex, "%x", rpm1);
    sprintf(rpm2_hex, "%x", rpm2);
