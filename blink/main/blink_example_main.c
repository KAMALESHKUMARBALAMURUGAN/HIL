...
void configure_uart(void) {
    ...
    uart_driver_install(ECHO_UART_PORT_NUM, 1024, 0, 0, NULL, 0);
}
...
void app_main(void) {
    ...
    uint8_t data[4];  // Buffer to store incoming data
    int value = 0;    // Variable to store the converted integer value

    while (1) {
        int len = uart_read_bytes(ECHO_UART_PORT_NUM, data, sizeof(data)-1, 20 / portTICK_PERIOD_MS);
        if (len > 0) {
            data[len] = '\0'; // Null-terminate whatever we received and treat it as a string
            value = atoi((char *)data); // Convert string to integer
